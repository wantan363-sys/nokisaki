...
type HistoryEntry = { id: string; recordType: string; date: string; label: string; name: string; qty: number; amount: number | null }
            ...
            {history.map((h, i) => (
              <div key={i} className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      if (!confirm(`「${h.name}」${h.qty}個の${h.label}を取り消しますか？\n在庫が元に戻ります。`)) return
                      const url =
                        h.recordType === 'sale' ? `/api/sales/${h.id}` :
                        h.recordType === 'purchase' ? `/api/purchases/${h.id}` :
                        `/api/stock_additions/${h.id}`
