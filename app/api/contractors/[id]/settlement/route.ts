...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabaseAdmin
    .from('settlements')
    .select('settled, picked_up')
    .eq('contractor_id', id)
    .eq('year', year)
    .eq('month', month)
    .maybeSingle()

  return NextResponse.json({ settled: data?.settled ?? false, picked_up: data?.picked_up ?? false })
}
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updateData: Record<string, boolean | string> = { [field]: value }
