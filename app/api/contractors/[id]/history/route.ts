...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  type Entry = { date: string; label: string; name: string; qty: number; amount: number | null }
  const entries: Entry[] = []
  ...
  for (const product of products ?? []) {
    ...
    for (const a of additions ?? []) {
      ...
      entries.push({ date: `${d.getMonth() + 1}/${d.getDate()}`, label: '補充', name: product.name, qty: a.quantity, amount: null })
    }
    ...
    for (const p of purchases ?? []) {
      ...
      entries.push({ date: `${d.getMonth() + 1}/${d.getDate()}`, label: p.type === 'half_buyout' ? '半値買取' : '仕入れ', name: product.name, qty: p.quantity, amount: p.quantity * p.unit_price })
    }
  }
